...
make_basefs() {
    setarch ${arch} mkarchiso-rt ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" init
    setarch ${arch} mkarchiso-rt ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "haveged intel-ucode memtest86+ mkinitcpio-nfs-utils nbd zsh" install
}


# Remove packages from base installation, e.g. the linux-kernel and install another version
# https://lists.archlinux.org/pipermail/arch-releng/2012-May/002537.html
##remove_base_packages() {
##    mkarchiso-rt ${verbose} -w "${work_dir}" -D "${install_dir}" -r "pacman -R --noconfirm linux" run
##}
...
make_packages_efi() {
### deprecated ###    setarch ${arch} mkarchiso-rt ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "prebootloader" install
    setarch ${arch} mkarchiso-rt ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "efitools" install
}
...
###kernel####     ARCHISO_GNUPG_FD=${gpg_key:+17} setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-linux -g /boot/archiso.img' run
    ARCHISO_GNUPG_FD=${gpg_key:+17} setarch ${arch} mkarchiso-rt ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-linux-rt -g /boot/archiso.img' run
    if [[ ${gpg_key} ]]; then
      exec 17<&-
    fi
}
...
make_customize_airootfs() {
    ...
    setarch ${arch} mkarchiso-rt ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/customize_airootfs.sh' run
...
####kernel ###    cp ${work_dir}/${arch}/airootfs/boot/vmlinuz-linux ${work_dir}/iso/${install_dir}/boot/${arch}/vmlinuz
    cp ${work_dir}/${arch}/airootfs/boot/vmlinuz-linux-rt ${work_dir}/iso/${install_dir}/boot/${arch}/vmlinuz
}
...
####kernel ####    gzip -c -9 ${work_dir}/${arch}/airootfs/usr/lib/modules/*-ARCH/modules.alias > ${work_dir}/iso/${install_dir}/boot/syslinux/hdt/modalias.gz
    gzip -c -9 ${work_dir}/${arch}/airootfs/usr/lib/modules/*-rt/modules.alias > ${work_dir}/iso/${install_dir}/boot/syslinux/hdt/modalias.gz
}
...
make_prepare() {
    ...
    setarch ${arch} mkarchiso-rt ${verbose} -w "${work_dir}" -D "${install_dir}" pkglist
    setarch ${arch} mkarchiso-rt ${verbose} -w "${work_dir}" -D "${install_dir}" ${gpg_key:+-g ${gpg_key}} prepare
...
####change it to build 32-bit image
